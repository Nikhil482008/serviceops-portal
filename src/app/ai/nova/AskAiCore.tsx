/* THE DRAWER'S OWN MARK — the earlier Core, kept for the one surface that asked for it.
 *
 * ⚠️ THIS IS THE SECOND ORB CONSTRUCTION IN THE TREE, and that is a deliberate exception, not an
 * oversight. Everything else — the 14px turn marks, the 8px label dots, the 26px edge handle, the
 * demo gallery — is `components/ui/siri-orb` through `NovaOrb`. This one is the FLIGHT LAYER's
 * orb: the single element that travels between the trigger, the greeting and the header seat.
 *
 * It cannot be narrowed to "the greeting" because the flight layer is one element by design —
 * opening the drawer is one object moving between two seats, and swapping the component halfway
 * would break the only thing that layer exists to do.
 *
 * Its structural weakness is unchanged and worth restating: it is authored at 120px and scaled by
 * `--orb-fit`, so its tone is tuned for one size. At the 18px header seat it needs the ramp
 * override below to read as anything at all. That is what `siri-orb` was brought in to fix, and
 * it is the price of this exception.
 */
...
export const CORE_BASE = 120;

/* THE STATE MODEL LIVES IN NovaOrb, and is imported rather than re-declared: two copies of a
   seven-state union is two places for them to disagree, and the drawer drives both marks from
   one `orbState`. */
/* ── THE PARTICLES ────────────────────────────────────────────────────────────────────────────
...
export function AskAiCore({
  state = 'idle', size = CORE_BASE, className = '', still = false, detail,
}: {
  state?: OrbState;
  ...
  return (
    <div
      ...
      style={{ width: size, height: size, ['--orb-fit' as string]: size / CORE_BASE }}
